...
readonly AUTHORITY_DIR="${AUTHORITY_DIR:-/opt/spire-demo/authority}"
readonly VERSION_FILE="${AUTHORITY_DIR}/config/version.env"
...
curl --fail --location --retry 3 --output "${DOWNLOAD_DIR}/${ARCHIVE_NAME}" "${ARCHIVE_URL}"

echo "[spire] Baixando checksum oficial..."
curl --fail --location --retry 3 --output "${DOWNLOAD_DIR}/${CHECKSUM_NAME}" "${CHECKSUM_URL}"
...
install -d -m 0755 "${SPIRE_INSTALL_DIR}/bin"

echo "[spire] Instalando binários..."
install -m 0755 "${EXTRACTED_DIR}/bin/spire-server" "${SPIRE_INSTALL_DIR}/bin/spire-server"
install -m 0755 "${EXTRACTED_DIR}/bin/spire-agent" "${SPIRE_INSTALL_DIR}/bin/spire-agent"

echo "[spire] Criando links em /usr/local/bin..."
ln -sfn "${SPIRE_INSTALL_DIR}/bin/spire-server" /usr/local/bin/spire-server
ln -sfn "${SPIRE_INSTALL_DIR}/bin/spire-agent" /usr/local/bin/spire-agent
